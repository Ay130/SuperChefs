...
function SelectBox({ children, wide = false, value, onChange }: { children: React.ReactNode; wide?: boolean; value?: string; onChange?: (value: string) => void }) {
  return <label className={`relative flex items-center justify-between gap-5 border border-slate-300 bg-white px-3 py-2 text-left text-[12px] text-slate-800 ${wide ? 'min-w-64' : 'min-w-40'}`}>
    <select aria-label={typeof children === 'string' ? children : 'Select option'} value={value} onChange={(event) => onChange?.(event.target.value)} className="absolute inset-0 h-full w-full cursor-pointer appearance-none bg-transparent opacity-0">
      <option>{children}</option><option>All Branches</option><option>All Departments</option><option>All Statuses</option><option>Bakery</option><option>Kitchen</option><option>Production</option>
    </select>
    <span>{children}</span><ChevronDown className="h-4 w-4 shrink-0" />
  </label>;
}
    ...
    <div className="flex h-screen min-w-[1000px] bg-[#f8faf9] text-[#101d18]">
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header className="flex h-[66px] shrink-0 items-center justify-between border-b border-slate-200 bg-white px-5">
          <div className="flex items-center gap-3"><button aria-label="Toggle sidebar" className="lg:hidden" onClick={() => setSidebarOpen(!sidebarOpen)}><Menu className="h-5 w-5" /></button><h1 className="text-xl font-bold tracking-tight">{active === 'New Request' ? 'NEW PROCUREMENT REQUEST' : active.toUpperCase()}</h1>{active === 'New Request' && <span className="bg-[#dff1e5] px-2 py-1 text-[11px] font-bold text-[#12633e]">EXCEL LIKE</span>}</div>
          <div className="relative flex items-center gap-5"><button aria-label="Notifications" onClick={() => showNotice('No new notifications')}><Bell className="h-5 w-5" /></button><button aria-label="Open profile menu" onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2"><div className="h-8 w-8 rounded-full bg-slate-200" /><div className="text-left text-[11px] font-bold">Aisha Bello<div className="text-[9px] font-normal text-slate-500">Procurement Officer</div></div><ChevronDown className="h-4 w-4" /></button>{profileOpen && <div className="absolute right-0 top-10 z-20 w-40 border border-slate-200 bg-white p-2 text-xs shadow-lg"><button onClick={() => showNotice('Profile settings coming soon')} className="w-full p-2 text-left hover:bg-slate-50">Profile settings</button><button onClick={onLogout} className="w-full p-2 text-left text-red-600 hover:bg-slate-50">Logout</button></div>}</div>
        </header>

        <div className="flex-1 overflow-auto p-5">
          {notice && <div role="status" className="fixed right-5 top-20 z-30 flex items-center gap-2 border border-emerald-300 bg-emerald-50 px-4 py-3 text-xs text-emerald-800 shadow-lg"><Check className="h-4 w-4" />{notice}</div>}
          {active === 'New Request' ? <NewRequest quantities={quantities} setQuantities={setQuantities} totalNeeded={totalNeeded} submitted={submitted} setSubmitted={setSubmitted} onNotice={showNotice} onNavigate={setActive} /> : <ListView active={active} onNotice={showNotice} />}
        </div>
...
function NewRequest({ quantities, setQuantities, totalNeeded, submitted, setSubmitted, onNotice, onNavigate }: { quantities: Record<number, number>; setQuantities: React.Dispatch<React.SetStateAction<Record<number, number>>>; totalNeeded: number; submitted: boolean; setSubmitted: (value: boolean) => void; onNotice: (message: string) => void; onNavigate: (section: string) => void }) {
  return <>
    ...
    <div className="mb-3 flex justify-end gap-3"><label className="flex cursor-pointer items-center gap-2 border border-[#7da994] bg-white px-4 py-2 text-[12px] font-bold text-[#12633e]"><Upload className="h-4 w-4" />Import from Excel<input type="file" accept=".csv,.xlsx" className="sr-only" onChange={(event) => event.target.files?.[0] && onNotice(`${event.target.files[0].name} selected for import`)} /></label><button onClick={() => onNavigate('Materials (Master)')} className="flex items-center gap-2 bg-[#007c46] px-5 py-2 text-[12px] font-bold text-white"><Package className="h-4 w-4" />View Material Master</button></div>
    ...
    <div className="mt-6 flex items-center justify-between"><button onClick={() => onNotice('Draft saved successfully')} className="flex items-center gap-2 border border-[#7da994] bg-white px-16 py-2 text-[12px] font-bold text-[#12633e]"><FileText className="h-4 w-4" />Save Draft</button><div className="flex gap-3"><button onClick={() => { setQuantities({}); setSubmitted(false); onNotice('Request form cleared'); }} className="flex items-center gap-2 border border-slate-300 bg-white px-10 py-2 text-[12px] font-bold"><RefreshCw className="h-4 w-4" />Clear</button><button onClick={() => setSubmitted(true)} className="flex items-center gap-2 bg-[#007c46] px-14 py-2 text-[12px] font-bold text-white"><Send className="h-4 w-4" />Submit Request</button></div></div>
  </>;
}

function ListView({ active, onNotice }: { active: string; onNotice: (message: string) => void }) {
  const [query, setQuery] = useState('');
  const [status, setStatus] = useState('All Statuses');
  const baseRows = active === 'Suppliers' ? [['ABC Foods Ltd', 'Mr. John Doe', '0803 123 4567', 'info@abcfoods.com', 'Active'], ['Fresh Supplies Co.', 'Mrs. Funke James', '0802 987 6543', 'info@freshsupplies.com', 'Active'], ['Bakers World', 'Mr. David Oho', '0809 456 7690', 'david@bakersworld.com', 'Active']] : materials.slice(0, 8).map((row, index) => [`PR-0002${4 - index}`, '05/08/2024', 'Lekki Branch', index % 2 ? 'Bakery' : 'Production', index % 2 ? 'Approved' : 'Pending Approval']);
  const rows = baseRows.filter((row) => row.join(' ').toLowerCase().includes(query.toLowerCase()) && (status === 'All Statuses' || row.includes(status)));
  return <div className="border border-slate-300 bg-white"><div className="flex items-center justify-between border-b border-slate-200 p-4"><div className="flex items-center gap-3"><div className="flex items-center border border-slate-300 px-3 py-2"><Search className="mr-2 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-64 text-[12px] outline-none" placeholder={`Search ${active.toLowerCase()}...`} /></div><SelectBox value={status} onChange={setStatus}>All Statuses</SelectBox></div><button onClick={() => onNotice(`${active === 'Suppliers' ? 'Supplier' : 'Material'} form opened`)} className="flex items-center gap-2 bg-[#007c46] px-4 py-2 text-[12px] font-bold text-white"><Plus className="h-4 w-4" />Add {active === 'Suppliers' ? 'Supplier' : 'Material'}</button></div><table className="w-full text-left text-[12px]"><thead className="bg-[#f0f6f2] font-bold"><tr>{(active === 'Suppliers' ? ['#', 'Supplier Name', 'Contact Person', 'Phone', 'Email', 'Status', 'Actions'] : ['Request No.', 'Date', 'Branch', 'Department', 'Status', 'Total Items', 'Total Equivalent', 'Actions']).map((h) => <th key={h} className="border-b border-slate-200 px-4 py-3">{h}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index} className="border-b border-slate-100"><td className="px-4 py-3">{active === 'Suppliers' ? index + 1 : row[0]}</td>{row.slice(1).map((cell, cellIndex) => <td key={cellIndex} className={`px-4 py-3 ${cell === 'Active' || cell === 'Approved' ? 'font-semibold text-[#13804b]' : ''}`}>{cell}</td>)}<td className="px-4 py-3"><button aria-label={`Open actions for row ${index + 1}`} onClick={() => onNotice(`Actions opened for ${row[0]}`)} className="font-bold text-[#13804b] hover:text-[#005c3b]">•••</button></td></tr>)}</tbody></table><div className="flex justify-between p-4 text-[11px] text-slate-500">Showing 1 to {rows.length} entries <span>‹ &nbsp; <b className="bg-[#007c46] px-2 py-1 text-white">1</b> &nbsp; 2 &nbsp; 3 &nbsp; ›</span></div></div>;
}
